...
- **无头回退带一个按 host 的熔断（`ScrapeCache::headless_futile`，默认 24h，`HEADLESS_FUTILE_TTL_SECS` 可配）。** 生产容器 `mem_limit: 1g`，Chrome 是里面最大的一坨；回退失败一次就把整个 host 记下来，同站后续 URL 直接跳过。没有它，一次全量重抓下几百条 B 站链接会各启一次 Chrome —— 反爬拒的是这台机器，不是那条路径，第一次就该得出结论。**TTL 必须长于调用方的巡检周期**：原值 900s 是照着「一次全量重抓」的时间尺度定的，而线上真实的重复请求由 API 侧的**小时级**巡检驱动（整点活性巡检、半点失活重试），15 分钟的熔断每次都已经过期，等于不存在。2026-08-07 查生产 `scrapper_call_log`：`console.cloud.tencent.com` 7 次调用 7 次失败，每次平均 28.6s，全烧在一个必然打不开的登录墙 SPA 上。回归测试 `headless_futile_ttl_outlives_an_hourly_sweep`。
