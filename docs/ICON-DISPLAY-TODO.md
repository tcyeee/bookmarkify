...
### 上线当天发现并修掉的第 ⑤ 条：判定依赖入参顺序

生产首次跑存量重算，第一次改 205 行，**紧接着空跑仍报 2 行待改** —— 幂等性不成立，而
「空跑确认变更量」这个动作的全部依据就是幂等。

根因：`maxWithOrNull` 在完全同分的候选里取「第一个」，于是结果取决于**入参顺序**。而顺序
根本不可靠 —— 抓取时是 scrapper 的返回序，重算时是数据库的**堆序**，一行被 UPDATE 过之后
新版本元组落在堆的另一处，扫描顺序就此改变（与 `UserLayoutNodeServiceImpl.CHILD_ORDER`
那条注释描述的是同一个 Postgres 行为）。

同分是常态不是边角：`01528e1f` 那个站点声明了两张矢量图标（`<link rel=icon>` 与
`<link rel=mask-icon>` 各一张 svg），两者 `effectiveSize()` 同为 `Int.MAX_VALUE`、
quality 同为 TRUSTED，一个字都分不开。

修法是给 `assignRoles` 第三、四遍与 `resolve` / `resolveCover` 的比较器统统加上
`.thenBy { it.id }`。**读侧那几处同样重要**：不加的话同一个用户在不同请求可能看到不同图标，
症状是图标「偶尔换一张」，几乎不可能被报成 bug。回归测试
`judgement does not depend on input order` 打乱同一批资产比对指纹。

症状本身很轻（`is_primary` 只影响同分时的先后，判定三档一字未动），但这是**规则自称纯函数
却不是纯函数**，值得单独记一条。
...
3. 去掉 `dryRun` 实跑；
4. **再空跑一次，`changed` 必须为 0** —— 幂等是这条流程的前提，不为 0 说明判定还依赖入参顺序（第 ⑤ 条）；
5. 打开「图标判定总览」复测，应为 **79 / 0 / 74 / 10，改进空间 0**；
6. **人眼看一遍置顶区。** 这一轮放行了 52 张此前不显示的图，而「好不好看」没有任何技术指标 —— 与截图那个「只截出裸 HTML」的老 bug 同理（见根 `CLAUDE.md`）。

**已于 2026-08-17 18:14 完成部署并执行**（三条流水线全绿；重算 205 行 / 185 个归属；
生产复测 79 / 0 / 74 / 10，改进空间 0）。第 4 步就是在这次执行中报出 `changed=2` 的。
