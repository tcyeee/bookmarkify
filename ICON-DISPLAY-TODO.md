...
> 决策日期：2026-08-17 · 状态：**阶段一已完成并上线（含生产核对），阶段二待讨论**
...
- [x] **部署 API → 再应用 drop 迁移**（2026-08-17 已完成，顺序正确）
- [x] 用 §1 的 SQL 复测，三档数字**一字不差**：27 / 40 / 86

### 上线后核对（2026-08-17，生产实测）

| 检查项 | 结果 |
|---|---|
| `site_display_pref` 表 / `uk_site_display_pref_owner` 索引 | 均已不存在 |
| §1 复测三档 | 27 / 40 / 86，与基线一致 —— 阶段一确实零行为变化 |
| 日志里 `site_display_pref` / `relation does not exist` | **0 条** |
| 500 / `SQLException` / `BadSqlGrammar` | **0 条** |
| `/bookmark/query`（桌面读路径，即本次迁移的唯一风险点） | 3 次真实调用，无异常 |

**部署窗口里 `SingleInstanceGuard` 报了 3 分钟 ERROR，与本次改动无关，且已自愈。** 老容器是被强杀的，
没走 `@PreDestroy` 的 `releaseOwnership()`，于是 Redis 里那把 3 分钟 TTL 的租约要等自然过期；
13:04–13:06 每分钟一条 error，13:07:30 新实例 `已取得单实例所有权`。**每次部署都会有这一段**，
它是设计内的（TTL 3min > 续约 1min），看到不必处理。
