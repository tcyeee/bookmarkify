<!-- <template>
  <div>
    <el-upload :headers="pageData.headers" :action="pageData.uploadUrl" :show-file-list="false" list-type="text">
      <button class="cy-btn cy-btn-accent">
        <svg width="24" height="24" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M28 40H7C5.34315 40 4 38.6569 4 37V11C4 9.34315 5.34315 8 7 8H41C42.6569 8 44 9.34315 44 11V23.0588" stroke="#333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
          <path d="M39 41L39 29" stroke="#333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
          <path d="M34 34L39 29L44 34" stroke="#333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
          <path d="M4 11C4 9.34315 5.34315 8 7 8H41C42.6569 8 44 9.34315 44 11V20H4V11Z" fill="none" stroke="#333" stroke-width="4" />
          <circle r="2" transform="matrix(-1.31134e-07 -1 -1 1.31134e-07 10 14)" fill="#333" />
          <circle r="2" transform="matrix(-1.31134e-07 -1 -1 1.31134e-07 16 14)" fill="#333" />
        </svg>
        <span>上传书签</span>
      </button>
    </el-upload>
  </div>
</template>

<script lang="ts" setup>
const userStore = useBaseStore();

const pageData = reactive<{
  file?: File;
  uploadUrl: string;
  headers: Object;
}>({
  uploadUrl: "/api/bookmark/upload",
  headers: { Authorization: `Bearer ${userStore.profile?.token}` },
});
</script> -->