<template>
  <footer class="w-full bg-black text-gray-200">
    <div class="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-2 font-medium">
        <Icon icon="mdi:monitor" class="size-[19px] text-gray-300 hover:text-white" />

        <span>Bookmarkify</span>
        <span class="text-gray-400">· 探索互联网更多可能</span>
      </div>
      <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs sm:text-sm text-gray-400">
        <a
          href="https://beian.miit.gov.cn"
          target="_blank"
          rel="noreferrer"
          class="flex items-center gap-1 transition-colors hover:text-white">
          浙ICP备18053383号-3
        </a>
        <a href="mailto:tcyeee@outlook.com" class="flex items-center gap-1 transition-colors hover:text-white">
          tcyeee@outlook.com
        </a>
      </div>

      <a
        href="https://wj.qq.com/s2/25315374/f339"
        target="_blank"
        rel="noreferrer"
        class="flex items-center gap-1 text-gray-400 transition-colors hover:text-white">
        <Icon icon="mdi:chat-processing" class="size-[19px] text-current" />
        <span>问题反馈</span>
      </a>
    </div>
  </footer>
</template>
