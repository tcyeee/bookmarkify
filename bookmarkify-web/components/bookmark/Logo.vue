<template>
  <div class="bookmark-icon">
    <!-- 如果网站有LOGO -->
    <el-image
      v-if="bookmark.iconActivity"
      :src="bookmark.iconUrlFull"
      @error="imageLoadFailed"
      :class="sm ? 'w-ico-dir h-ico-dir' : bookmark.iconHd ? 'w-app h-app' : 'w-ico h-ico'">
      <template #placeholder>
        <svg
          :class="sm ? 'w-ico-dir h-ico-dir' : 'w-ico h-ico'"
          viewBox="0 0 48 48"
          fill="none"
...
function dataInit() {
  ...
  props.bookmark.iconUrlFull = item.iconUrlFull ? item.iconUrlFull : getBaseAvatarUrl(item.urlFull)
}
