...
> **Spike 结论（2026-06-21）**：已用临时页 `pages/spike-cross-grid.vue`（两个相邻 Vuuri 网格设同一 `group-id="spike"`，网格 B 外套 `backdrop-filter: blur` 卡片）实测：跨网格拖拽**双向均跟手、无坐标错位**，从毛玻璃卡片往外拖也正常。**结论：vuuri group 跨网格在浮层/blur 结构下默认即可用，无需强制 `dragContainer=body`，无需改造毛玻璃为兄弟层。** 后续任务采用 vuuri 内置 group 默认行为即可。
