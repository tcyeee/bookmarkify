...
| 移出到根目录（文件夹剩余 ≥ 2） | 源文件夹（已移除该节点） | 推送源文件夹 |
| 移出到根目录（文件夹剩余 1） | 完整桌面布局（ROOT 节点） | 推送完整布局，前端需全量刷新 |
| 文件夹间移动（源文件夹剩余 ≥ 2） | 目标文件夹（含新增节点） | 分别推送源文件夹和目标文件夹 |
| 文件夹间移动（源文件夹剩余 1） | 完整桌面布局（ROOT 节点） | 推送完整布局，前端需全量刷新 |

> **文件夹自动解散规则**：当某个节点移出文件夹后，若该文件夹仅剩 1 个节点，服务端会自动将最后一个节点移到根目录、继承文件夹的 `sort` 值（保持在桌面上的位置不变），然后删除该文件夹。此时响应体 `data.id === 'ROOT'`，前端应以此为信号执行完整布局刷新。
