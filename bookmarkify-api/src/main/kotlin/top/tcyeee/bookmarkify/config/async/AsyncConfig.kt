...
class AsyncConfig {
     ...
     * 页面截图补抓线程池。
     *
     * **单线程**：截图强制走无头浏览器，而对端的无头并发有上限（scrapper 的
     * `HEADLESS_CONCURRENCY`，默认 2；生产容器 1GB 内存 / 2 vCPU）。这边多开线程并不会更快
     * ——多出来的请求只会堵在对端的准入队列里，等 `HEADLESS_QUEUE_WAIT_SECS` 到点后拿回一个
     * `HEADLESS_UNAVAILABLE`。并发度必须与对端对齐；对端从"一次一个 Chrome"放宽到 2 之后，
     * 这里是否跟着放宽是可以量的（先看截图的实际耗时分布），但不要拍脑袋改。
     *
