...
data class BookmarkWrapper(
    ...
    var xUaCompatible: String? =
        null, // 兼容模式，如 IE 浏览器兼容设置，来自 <meta http-equiv="X-UA-Compatible">
    ...
    var canonicalUrl: String? =
        null, // Canonical 链接，用于指定页面标准地址，避免SEO重复内容，来自 <link rel="canonical">
    var manifestUrl: String? = null, // 网站 manifest 文件路径，定义PWA相关配置，来自 <link rel="manifest">
    var appleTouchIcons: Map<String, String> =
        emptyMap(), // Apple Touch Icons，适配iOS主屏幕图标，多尺寸时可用尺寸为key，URL为value
    var preconnectUrls: List<String> =
        emptyList(), // 预连接域名，用于提升首次请求速度，来自 <link rel="preconnect">
    var preloadResources: List<PreloadResource> =
        emptyList(), // 预加载资源，如js、css、图片资源URL，来自 <link rel="preload">
    var dnsPrefetchUrls: List<String> =
        emptyList(), // DNS 预取，用于提前解析域名，来自 <link rel="dns-prefetch">
    var styleSheets: List<String> =
        emptyList(), // 样式表链接，CSS 文件 URL 列表，来自 <link rel="stylesheet">
    var scriptPrefetchUrls: List<String> =
        emptyList(), // 预获取的脚本文件，来自 <link rel="prefetch" as="script">
...
data class ManifestIcon(
    ...
    fun size(): Int {
        ...
        return sizes.split(Regex("\\s+"))
            .mapNotNull { s -> s.split(Regex("[xX]")).firstOrNull()?.toIntOrNull() }
            .maxOrNull() ?: 0
    }
}
