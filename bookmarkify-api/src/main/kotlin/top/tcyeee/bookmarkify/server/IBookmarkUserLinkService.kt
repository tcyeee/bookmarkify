...
interface IBookmarkUserLinkService : IService<BookmarkEntity> {
    ...
    fun resetPageId(uid: String, userLinkId: String, pageId: String): Boolean

    /**
     * 把「等着被绑定」的占位标记(`page_id = 'LOADING'`)清成 NULL，表示这条记录**确定**
    ...
    /** 返回用户所有未删除书签关联的 pageId 集合 */
    fun bookmarkIdsByUid(uid: String): Set<String>
    /** 返回用户被添加了多次的 pageId 集合(同一站点收藏了 ≥2 次)，用于"重复书签"筛选 */
    fun duplicatePageIds(uid: String): Set<String>
