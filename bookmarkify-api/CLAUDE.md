...
| `site_asset` / `page_meta` / `scrape_snapshot` | Crawl results, one writer (`SiteAssetWriter`), crawl facts only (replaced `bookmark_logo`; see root `CLAUDE.md`). A fourth, `site_display_pref`, held admin icon tweaks and was dropped on 2026-08-17 — icon quality is a rules problem, see `docs/ICON-DISPLAY-TODO.md` |
...
**Duplicate tiles are allowed (2026-08-31).** A user can save the same canonical page many times — `addOne` / `linkOne` no longer pre-check, `uk_bookmark_uid_page` was dropped (`2026-08-31_allow_duplicate_bookmarks.sql`), and the import bind path lost its `discardDuplicatePlaceholder` fallback. `E126` is retired. `insertNodeAndLink` keeps its transaction purely for the node+link atomicity guarantee. The canonical quadruple still matters — it decides which `page` scrape row a bookmark *shares* — it just no longer rejects a new bookmark. Cleanup is user-driven via the `duplicatesOnly` filter (`duplicatePageIds`).
...
**A sweep probes the whole batch first, then writes.** In between sits `LivenessPolicy.breakerReason`, which aborts the round if ≥50% of the round's **domains** came back `UNKNOWN` (subject to the two guards below) or ≥90% came back `DEAD` (≥20 probes). The second rule catches the case the first cannot: the scrapper is up but its egress is broken, so it *honestly* reports `alive=false` for everything. Ping logs are still written on an aborted round — that batch is the evidence of what broke. Order matters; probing and writing in one pass makes the breaker useless.

**`UNKNOWN` is two different things, and only one of them is breaker evidence.** `outcomeOf` folds both `403/406/412/…` (the origin answered — it's up, just refusing our datacenter IP) and `blocked` / no-status / scrapper-unreachable / `load_shed` (our chain reached nothing) into `PingOutcome.UNKNOWN`. Only the second is a symptom of *our* failure. `pingWebsite` now returns `PingProbeResult(outcome, siteRefusal)` — `siteRefusal` is set from `LivenessPolicy.isSiteRefusal` — and `HostOutcome.ourChainInconclusive` carries the distinction into `breakerReason`. The breaker's UNKNOWN rule:
- **numerator** = domains whose every probe is our-chain `UNKNOWN`;
- **denominator** = all domains *minus* those whose every probe is a site-refusal (permanent anti-bot like `bilibili`/`zfrontier` is constant noise; getting a status code back proves our egress works). `ALIVE` domains stay in the denominator — they're a real "we probed something and our chain was fine" sample, so they dilute the ratio as before;
- an **absolute floor** `MIN_UNKNOWN_HOSTS` = 4: a pure ratio on a 2–3 domain denominator is noise.

2026-08-28 production is why: a small deployment's overdue candidates only ever touched 3–4 domains, two of them permanent anti-bot. `2/4 = 50%` tripped *every* round, `livenessCheckStaleBookmarks` aborted half its rounds, and that batch's liveness + content refresh never advanced — while the scrapper was fine. Cost of the floor: a genuinely tiny deployment (<4 non-refusing domains) hitting a real scrapper outage won't trip *this* rule — but the `DEAD ≥ 90%` rule is untouched, and `UNKNOWN` never writes `UNREACHABLE` anyway (its only teeth are re-crawl dispatch, which `parseBudget` backpressure already caps). Regression: `小体量部署里两个永久反爬域名不足以熔断` (denominator exclusion), `我方探不到的域名数没到下限…` + `…达到下限且过半就照常熔断` (floor boundaries), `isSiteRefusal …` in `LivenessPolicyTest`.

**Consecutive breaker rounds escalate the standing alert.** `SweepHealthVO.maxConsecutiveBreaker` counts, per `task_label`, how many of the most-recent rounds aborted in a row (a single breaker is "our chain hiccuped"; a run of them is the self-sustaining stall — an aborted round doesn't advance the effective cursor, so the next round re-selects the same batch and reaches the same verdict). `SweepBreakerAlert.vue` rewrites its title/description once the run hits `STUCK_BREAKER_ROUNDS` (3). This is what turns "the 2026-08-10 deadlock shape" from silent into loud.
...
**Content refresh is what makes this an "update" mechanism.** `shouldRefreshContent` re-scrapes when `last_parse_at` is older than `contentRefreshIntervalDays` (default 30). The old condition (`alive && !isActivity`) was dead code — `isActivity=false` never coexists with `parse_status=SUCCESS` — so healthy sites were never re-crawled at all. It fires on a `ping` outcome of `ALIVE` **or** a site-refusal `UNKNOWN` (`403/406/412` — the site is up, anti-bot just blocked our `HEAD` probe): the scrape path is strictly more capable than a bare `HEAD` (headless fallback + `siteapi.rs` official-API rescue), so those pages deserve the attempt. Gating on `== ALIVE` alone left `bilibili` video pages — forever `412` to our datacenter IP, hence forever `UNKNOWN` from a ping — with permanently stale titles/covers, despite `siteapi.rs` existing for exactly them.
...
- Icons live in `site_asset` and are **not** joined by any `BookmarkShow` SQL. **Build them through `IconResolver.decorate(shows, mode)`** — it resolves and injects in one call, including the second `tileLogo` copy the pinned row needs. `initDisplay` takes a `DisplayIcons` (mode + both resolved icons) and has no default, precisely because the earlier `logo` default let several call sites forget, silently degrading the whole desktop to monogram tiles. Bundling the mode with its own resolution also makes "pick the icon by TILE but the caption by LIST" unrepresentable; it used to be two parameters held together by a comment.
