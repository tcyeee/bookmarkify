...
| `bookmark_sweep_log` | One row per **sweep round** — candidates/backlog/outcome counts/deferred/breaker reason. The sweep's only SLI; see "Liveness sweeps" |
...
| `reviveArchivedBookmarks()` | daily, 02:00 | `ARCHIVED` + `next_check_at <= now()` | Keeps archival from being a one-way door |
...
**`ARCHIVED` is not a one-way door.** `reviveArchivedBookmarks` (daily, batch 50) re-probes archived rows and re-crawls the ones that answer. Everything that pushes a row into archival is automated and fallible (a temporary DNS change, or the target blackholing our egress IP for a while, is enough); an automatic entrance with no automatic exit turns one misjudgement into permanent deletion. Archival also now sets `next_check_at = now + 30d` instead of leaving the cursor in the past.
