...
const formWrapperRef = useTemplateRef<HTMLElement>("formWrapperRef");
...
function updateFormHeight() {
  formHeight.value = formWrapperRef.value?.offsetHeight ?? 0;
}

const enableCollapseButton = computed(() => formHeight.value > 300);
...
watch(
  () => ({
    ...
    canCollapse: enableCollapseButton.value,
  }),
  (state) => {
    formApi.setState((prev) => {
      ...
      return {
        ...finalFormOptions,
        showCollapseButton: state.canCollapse,
        collapseTriggerResize: !!state.canCollapse,
      };
    });
  },
  {
  ...
  <div :class="cn('bg-card h-full rounded-md', className)">
    <VxeGrid ref="gridRef" :class="
      ...
      <template #toolbar-tools="slotProps">
        ...
        <VxeButton icon="vxe-icon-search" circle class="ml-2" v-if="gridOptions?.toolbarConfig?.search && !!formOptions" :status="showSearchForm ? 'primary' : undefined" :title="'搜索'" @click="onSearchBtnClick" />
      </template>

      <!-- form表单 -->
      <template #form>
        <div v-if="formOptions" v-show="showSearchForm !== false" ref="formWrapperRef" :class="
            cn(
          ...
          <div v-if="isSeparator" :style="{
            ...
            }" class="bg-background-deep z-100 absolute -left-2 bottom-1 h-2 w-[calc(100%+1rem)] overflow-hidden md:bottom-2 md:h-3"></div>
        </div>
      </template>
      <!-- loading -->
      <template #loading>
